...
class DuelingNetWork(nn.Module):
    ...
    def forward(self, ope_fea, ope_adj, job_adj, mach_adj, job_select, h, c):
        batch_num = job_select.shape[0]
        job_num = job_select.shape[1]
        ope_gat = self.state_gat(ope_fea, ope_adj)
        job_fea = self.job_gat(ope_fea, job_adj)
        mach_fea = self.mach_gat(ope_fea, mach_adj)
        ope_select = ope_gat[(torch.tensor(range(batch_num),dtype=torch.int64).repeat(job_num).view(-1),
                              job_select.view(-1))].view(batch_num, job_num, -1)
        ...
        job_ope_fea = torch.cat([job_fea, ope_select], dim=-1)
        mach_num = mach_fea.shape[1]
        mach_dim_num = mach_fea.shape[-1]
        job_dim_num = job_ope_fea.shape[-1]
        action = T.cat([job_ope_fea.repeat(1, 1, mach_num).view(-1, mach_num * job_num, job_dim_num),
                        mach_fea.repeat(1, job_num, 1).view(-1, mach_num * job_num, mach_dim_num)], dim=-1)
...
class DuelingDQN:
    def __init__(self, alpha, state_dim, action_dim, hidden_dim, hidden_layer,
        ...
        self.memory = GReplayBuffer.ReplayBuffer(max_size, batch_size, ope_num, machine_num, gat_output_dim)
    ...
    def choose_action(self, state, h, c, isTrain=True):
        ...
        if (np.random.random() < self.epsilon) and isTrain:
            ...
            ope_fea = T.tensor(state[0], dtype=T.float).to(device)
            ope_adj = T.tensor(state[1], dtype=T.int32).to(device)
            job_adj = T.tensor(state[2], dtype=T.int32).to(device)
            mach_adj = T.tensor(state[3], dtype=T.int32).to(device)
            job_select = T.tensor(state[6], dtype=T.int64).to(device)
            h = T.tensor(h, dtype=T.float).to(device)
            c = T.tensor(c, dtype=T.float).to(device)
            _, A, hn, cn = self.q_eval.forward(ope_fea, ope_adj, job_adj, mach_adj, job_select, h, c)
        else:
            ope_fea = T.tensor(state[0], dtype=T.float).to(device)
            ope_adj = T.tensor(state[1], dtype=T.int32).to(device)
            job_adj = T.tensor(state[2], dtype=T.int32).to(device)
            mach_adj = T.tensor(state[3], dtype=T.int32).to(device)
            job_select = T.tensor(state[6], dtype=T.int64).to(device)
            action_mask=T.tensor(state[4], dtype=T.float).to(device)
            h = T.tensor(h, dtype=T.float).to(device)
            c = T.tensor(c, dtype=T.float).to(device)
            _, A, hn, cn = self.q_eval.forward(ope_fea, ope_adj, job_adj, mach_adj, job_select, h, c)
            A = action_mask + A
    ...
    def learn(self):
        ...
        [ope_fea, ope_adj, job_adj, mach_adj, action_mask, ope_select,
         h, c, action, reward,
         next_ope_fea, next_ope_adj, next_job_adj, next_mach_adj, next_action_mask, next_ope_select,
        ...
        job_adj_tensor = T.tensor(np.array(job_adj), dtype=T.float).to(device)
        mach_adj_tensor = T.tensor(np.array(mach_adj), dtype=T.float).to(device)
        ope_select_tensor = T.tensor(np.array(ope_select), dtype=T.long).to(device)
        ...
        next_ope_fea_tensor = T.tensor(np.array(ope_fea), dtype=T.float).to(device)
        next_ope_adj_tensor = T.tensor(np.array(ope_adj), dtype=T.float).to(device)
        next_job_adj_tensor = T.tensor(np.array(job_adj), dtype=T.float).to(device)
        next_mach_adj_tensor = T.tensor(np.array(mach_adj), dtype=T.float).to(device)
        next_action_mask_tensor = T.tensor(np.array(action_mask), dtype=T.float).to(device)
        next_ope_select_tensor = T.tensor(np.array(ope_select), dtype=T.long).to(device)
        ...
        with T.no_grad():
            V_, A_, _, _ = self.q_target.forward(next_ope_fea_tensor,
                                                 next_ope_adj_tensor,
                                                 next_job_adj_tensor,
                                                 next_mach_adj_tensor,
                                                 next_ope_select_tensor,
                                                 hn_tensor,
                                                 cn_tensor)
            q_ = V_ + A_ - T.mean(A_, dim=-1, keepdim=True)
            # q_[terminals_tensor] = 0.0
            target = rewards_tensor + (1 - done_tensor) * self.gamma * T.max(q_ + next_action_mask_tensor, dim=-1)[0]

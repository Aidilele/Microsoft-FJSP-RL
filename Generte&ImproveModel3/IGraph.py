...
class FJSPGraph:
    ...
    def get_fea(self, ope_fea_dim=7, insert_fea_dim=4):
        info = {}
        ...
        ope_fea[0]=self.start_node.get_fea()
        ope_fea[-1]=self.end_node.get_fea()
        for job in self.ope_list:
            for ope in job:
    ...
    def excute_action(self, action):
        ...
        if self.trans2object(ope).process_machine==None:
            return -1

...
class DuelingNetWork(nn.Module):
    ...
    def forward(self, ope_fea, ope_adj, insert_adj, h, c, insert_fea=None):
        ...
        ope_gat = ope_gat[:, 1:-1]
        insert_gat = self.action_gat(ope_fea, insert_adj)  # 直接使用Node_Fea作为InsertFea的输入特征
...
class DuelingDQN:
    def __init__(self, alpha, state_dim, action_dim, hidden_dim, hidden_layer,
        ...
        self.memory = IReplayBuffer.ReplayBuffer(max_size, batch_size, ope_num, machine_num, gat_output_dim)
    ...
    def learn(self):
        ...
        self.summer.add_scalar('Iloss', loss, self.summer_step)
        self.summer_step += 1

...
def main():
    ...
    action_dim = gat_output_dim + ope_dim
    ...
    env = FJSPEnviroment(dir_path='./dataset/' + dataset,env_tpye='Train')
    env.reset()
    ...
    for episode in range(max_episodes):
        ...
        if ep_count % len(env.graph_list) == 0:
            ave_reward = ave_reward / len(env.graph_list)
            ave_decady = ave_decady / len(env.graph_list)
            ave_cmax = ave_cmax / len(env.graph_list)
            print(
            ...
            Summer.add_scalar("EP Reward", ave_reward, print_count)
            Summer.add_scalar("EP Decade", ave_decady, print_count)
            Summer.add_scalar("EP Cmax ", ave_cmax, print_count)
            ave_reward = 0
            ave_decady = 0
            ave_cmax = 0
            print_count += 1

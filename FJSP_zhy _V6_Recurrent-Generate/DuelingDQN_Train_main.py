...
def main():
    ...
    for episode in range(max_episodes):
        ...
        if ep_count % len(env.graph_list) == 0:
            ...
            Summer.add_scalar("EP Reward", ave_reward / len(env.graph_list), print_count)
            Summer.add_scalar("EP Decade", ave_decady / len(env.graph_list), print_count)
            Summer.add_scalar("EP Cmax ", ave_cmax / len(env.graph_list), print_count)
            ave_reward = 0
            ave_decady = 0
            ave_cmax = 0
            print_count+=1
